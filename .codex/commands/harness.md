...
- 하네스 규칙의 단일 원본은 이 파일이다. 별도의 `docs/CODEX_HARNESS.md`를 만들지 마라.
- 리뷰 규칙의 단일 원본은 `.codex/commands/review.md`다. 별도의 `docs/CODEX_REVIEW.md`를 만들지 마라.
- Claude CLI 옵션(`claude`, `--dangerously-skip-permissions`, `--output-format`)을 사용하지 마라. 실행기는 `codex exec`를 사용한다.

## 워크플로우

### A. 탐색

`/AGENTS.md`와 `/docs/` 하위 문서(PRD, ARCHITECTURE, ADR, UI_GUIDE 등)를 읽고 프로젝트의 기획, 아키텍처, 설계 의도를 파악한다.
...
4. **시그니처 수준 지시** - 함수/클래스의 인터페이스만 제시하고 내부 구현은 Codex 재량에 맡긴다. 단, 설계 의도에서 벗어나면 안 되는 핵심 규칙은 반드시 명시한다.
...
- `project`: 프로젝트명. `AGENTS.md`와 PRD를 참조한다.
...
- Codex 호출: `codex exec`를 비대화형으로 실행한다. Windows PowerShell 실행 정책 문제를 피하기 위해 기본값은 `codex.cmd`이며, 필요하면 `CODEX_BIN` 환경변수로 덮어쓸 수 있다.
